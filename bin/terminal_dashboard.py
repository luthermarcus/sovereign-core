...
def render_master_terminal():
    ...
    panel = Panel(
        table,
        title="[bold green]Sovereign Core v0.3.7 (Mining & Wallet Terminal)[/bold green]",
        subtitle="[dim]Options: [1] Mine DePIN Proof [2] Send Transfer [3] Diagnostics [4] Exit[/dim]",
        border_style="green"
    )
    return panel
...
if __name__ == "__main__":
    console.clear()
    rprint("[green]>[/green] Booting Sovereign Core Master Ecosystem Terminal...")
    time.sleep(1)
    
    try:
        while True:
            console.clear()
            console.print(render_master_terminal())
            rprint("\n[bold cyan]Master Ecosystem Actions:[/bold cyan]")
            rprint("  [1] Run DePIN Proof-of-Compute Mining Round")
            rprint("  [2] Send 5.0 FOX Micro-Payment (Faucet -> User)")
            rprint("  [3] Run Node Diagnostics & Self-Healing")
            rprint("  [4] Exit Terminal")
            
            choice = input("\nSelect action [1-4]: ").strip()
            if choice == "1":
                rprint("[yellow]>[/yellow] Executing CPU Proof-of-Compute mining puzzle...")
                res = node.mine_depin_proof()
                if res["status"] == "success":
                    rprint(f"[green]✓[/green] Block Mined! Hash: {res['hash'][:16]}... (Reward: +{res['reward']} FOX)")
                else:
                    rprint(f"[red]✗[/red] Mining round failed.")
                time.sleep(2.0)
            elif choice == "2":
                ...
                if res["status"] == "buffered":
                    ...
                    rprint(f"[green]✓[/green] Batch committed transfers to SQLite WAL ledger!")
                time.sleep(1.5)
            elif choice == "3":
                ...
                rprint("[yellow]Shutting down Sovereign Terminal. Node running in background.[/yellow]")
                break
            else:
                time.sleep(0.5)
